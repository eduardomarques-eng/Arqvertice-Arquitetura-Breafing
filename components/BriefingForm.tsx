...
import { CheckCircle2, Loader2, ChevronRight, ChevronLeft, Sparkles } from 'lucide-react';
...
export function BriefingForm() {
  ...
  const [predictionsApplied, setPredictionsApplied] = useState<string[]>([]);
  ...
  const {
    ...
    defaultValues: {
      ...
      deckVaranda: 'Não',
      trabalhaEmCasa: 'Não',
      recebeVisitas: 'Frequente',
      nivelDesejado: 'Médio padrão',
      flexibilidade: 'Média',
      inicioProjeto: 'Imediato',
      finalidade: 'Moradia',
      estiloDesejado: 'Contemporâneo',
      lavanderia: 'Sim',
      dependenciaServico: 'Não',
      deposito: 'Não',
      academia: 'Não',
      cinema: 'Não',
      adega: 'Não',
      ...
      automacao: 'Não',
      energiaSolar: 'Não',
      sistemaSeguranca: 'Não',
      climatizacao: 'Sim',
      matricula: 'Sim',
      escritura: 'Sim',
      planialtimetrico: 'Não',
      projetoAnterior: 'Não',
      // New defaults
      ...
      aumentarFamilia: 'Não',
      condominio: 'Sim',
      levantamentoTopografico: 'Não',
      sondagemSolo: 'Não',
      construirImediatamente: 'Sim',
      financiamentoAprovado: 'Não',
      aquecimentoAgua: 'Gás',
      iluminacao: 'Quente (Aconchegante)',
      frequenciaEventos: 'Finais de semana',
      prefereAmbientes: 'Integrados',
      prioriza: 'Conforto',
    }
  });
  ...
  useEffect(() => {
    const subscription = watch((value, { name, type }) => {
      if (type === 'change' && name) {
        // Exemplo 1: Casa Alto Padrão
        if (name === 'nivelDesejado' && value.nivelDesejado === 'Alto padrão') {
          if (!predictionsApplied.includes('altoPadrao')) {
            setValue('suites', '3');
            setValue('closet', 'Sim');
            setValue('areaGourmet', 'Sim');
            setValue('piscina', 'Sim');
            setValue('automacao', 'Sim');
            setValue('climatizacao', 'Sim');
            setPredictionsApplied(prev => [...prev, 'altoPadrao']);
          }
        }
        
        // Exemplo 2: Área > 250m2
        if (name === 'areaTerreno' && value.areaTerreno) {
          const area = parseInt(value.areaTerreno.replace(/\D/g, ''));
          if (area > 250 && !predictionsApplied.includes('areaGrande')) {
            setValue('lavabo', 'Sim');
            setValue('escritorio', 'Sim');
            setValue('varanda', 'Sim');
            setPredictionsApplied(prev => [...prev, 'areaGrande']);
          }
        }
      }
    });
    return () => subscription.unsubscribe();
  }, [watch, setValue, predictionsApplied]);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2 mt-2">
                      <FileInput 
                        ...
                        helperText="Formatos aceitos: Imagens, PDF, DWG. Máx 10 arquivos, 5MB cada."
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    ...
                    <div className="md:col-span-1">
                      <FileInput 
                        ...
                        multiple 
                        accept="image/*"
                        helperText="Envie até 10 fotos de projetos que você gosta. Máx 5MB por arquivo."
                      />
                    </div>
            ...
            {currentStep === 5 && (
              <>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <RadioGroup label="Deseja piscina externa?" registration={register('piscinaExterna')} options={[{ label: 'Sim', value: 'Sim' }, { label: 'Não', value: 'Não' }]} />
                    <RadioGroup label="Deseja churrasqueira/gourmet?" registration={register('churrasqueira')} options={[{ label: 'Sim', value: 'Sim' }, { label: 'Não', value: 'Não' }]} />
                    <RadioGroup label="Deseja jardim?" registration={register('jardim')} options={[{ label: 'Sim', value: 'Sim' }, { label: 'Não', value: 'Não' }]} />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2 mt-2">
                      <FileInput 
                        ...
                        helperText="Formatos aceitos: PDF, DWG, DOC, Imagens. Máx 10 arquivos, 5MB cada."
                      />
                    </div>
                  </div>
                </div>
